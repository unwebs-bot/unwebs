<?php
/**
 * Template Name: 포트폴리오 (Portfolio Index)
 *
 * 데이터: `inc/cm-portfolio-main.php` 코드 큐레이션 데이터셋
 * URL  : /portfolio/  (개별 케이스 /portfolio/{slug}/ 는 functions.php rewrite + page-portfolio-case.php)
 *
 * 메인 섹션(`template-parts/main/section-portfolio.php`)과 동일한 카드 구조를 재활용해
 * 디자인 일관성 유지. 신규 항목 추가는 `cm_portfolio_dataset()` 배열 편집.
 */
...
$portfolio_items = function_exists('cm_get_main_portfolio_items')
    ? cm_get_main_portfolio_items(50) // index는 전체 노출 — 상한만 안전 처리
    : array();
?>

<main class="cm-main" id="main-content" role="main">

  <h1 class="blind">제작 사례 포트폴리오</h1>

  <!-- ========== Hero ========== -->
  <section class="portfolio-intro-con sub-content-con cm-sub-hero">
    <div class="area">
  ...
  <!-- ========== 포트폴리오 그리드 ========== -->
  <section class="portfolio-list-con">
    <div class="area">

      <?php if (!empty($portfolio_items)) : ?>
      <ul class="main-portfolio-grid" data-animate="fade-up" data-delay="100">
      ...
      <?php else : ?>
      <p class="portfolio-empty">등록된 포트폴리오가 없습니다.</p>
      <?php endif; ?>
